...
#if 0
ptpgp_err_t
ptpgp_uri_parse(ptpgp_uri_t *u, 
                u8 *src,
                size_t src_len) {
  size_t i;
  u8 *p, *s;
  ptpgp_uri_state_t state;
  return PTPGP_OK;
  ...
  while (state != STATE(DONE)) {
    ...
    case STATE(INIT):
      ...
      /* check scheme length */
      if (s - src >= SIZE(SCHEME))
        DIE(SCHEME_TOO_LONG);

      /* copy/null-terminate scheme */
      memcpy(r->scheme, src, s - src);
      r->scheme[s - src] = 0;
    ...
    case STATE(HOST):
      ...
      /* if s == NULL, then the hostspec extends until the end of
       * the input string */
      TRY(parse_hostspec(src, s ? s - src : src_len));
