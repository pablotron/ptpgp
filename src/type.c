...
#define A(a) T(TYPE), PTPGP_TYPE_##a
  ...
  "Public Key Algorithm",       "public-key", 0, 0
}, {
  A(SYMMETRIC),                 R(MUST),      R(MUST),
  "Symmetric Algorithm",        "symmetric",  0, 0
}, {
  A(SYMMETRIC_MODE),            R(MUST),      R(MUST),
  "Symmetric Mode",             "mode",       0, 0
}, {
  A(COMPRESSION),               R(MUST),      R(MUST),
  "Compression Algorithm",      "compression", 0, 0
}, {
  A(HASH),                      R(MUST),      R(MUST),
  "Hash Algorithm",             "hash",       0, 0
}, {
  A(S2K),                       R(MUST),      R(MUST),
  "String-to-Key Specifier",    "s2k",        0, 0
}, {
#undef A
...
#define A(a) T(PUBLIC_KEY), PTPGP_PUBLIC_KEY_TYPE_##a
  ...
  "RSA (Encrypt-Only)",         "rsa-encrypt-only",         2, 4
}, {
  ...
  "Elliptic Curve (Reserved)",  "elliptic-curve",           0, 0
}, {
  A(ECDSA),                     R(MUST_NOT),  R(MUST_NOT),
  "ECDSA (Reserved)",           "ecdsa",                    0, 0
}, {
  A(ELGAMAL_ENCRYPT_OR_SIGN),   R(MAY),       R(MUST_NOT),
  "Elgamal (Deprecated)",       "elgamal-deprecated",       3, 1
}, {
  A(DH),                        R(MUST_NOT),  R(MUST_NOT),
  "Diffie-Hellman",             "dh",                       0, 0
}, FOOTER, {
#undef A
...
#define A(a) T(SYMMETRIC), PTPGP_SYMMETRIC_TYPE_##a
  ...
  "Plaintext",                  "plaintext",  0,    0
}, {
  A(IDEA),                      R(SHOULD),    R(SHOULD_NOT),
  "IDEA",                       "idea",       64,   128
}, {
  A(TRIPLEDES),                 R(MUST),      R(MUST),
  "TripleDES (DES-EDE)",        "tripledes",  64,   168 /* XXX */
}, {
  A(CAST5),                     R(SHOULD),    R(SHOULD),
  "CAST5 (128-bit)",            "cast5",      64,   128
}, {
  A(BLOWFISH),                  R(MAY),       R(MAY),
  "Blowfish (128-bit)",         "blowfish",   64,   128
}, {
  ...
  "AES (128-bit)",              "aes-128",    128,  128
}, {
  A(AES_192),                   R(MAY),       R(MAY),
  "AES (192-bit)",              "aes-192",    128,  192
}, {
  A(AES_256),                   R(MAY),       R(MAY),
  "AES (256-bit)",              "aes-256",    128,  256
}, {
  A(TWOFISH),                   R(MAY),       R(MAY),
  "Twofish (256-bit)",          "twofish",    128,  256
}, {
  A(CAMELLIA_128),              R(MAY),       R(MAY),
  "Camellia (128-bit)",         "camellia-128", 128, 128
}, {
  A(CAMELLIA_192),              R(MAY),       R(MAY),
  "Camellia (192-bit)",         "camellia-192", 128, 192
}, {
  A(CAMELLIA_256),              R(MAY),       R(MAY),
  "Camellia (256-bit)",         "camellia-256", 128, 256
}, FOOTER, {
#undef A

#define A(a) T(SYMMETRIC_MODE), PTPGP_SYMMETRIC_MODE_TYPE_##a
  A(NONE),                      R(MUST_NOT),  R(MUST_NOT),
  "None",                       "none",       0, 0
}, {
  A(ECB),                       R(MUST),      R(MUST),
  "Electronic Codebook",        "ecb",        0, 0
}, {
  A(CFB),                       R(MUST),      R(MUST),
  "Cipher Feedback",            "cfb",        0, 0
}, {
  A(CBC),                       R(MUST),      R(MUST),
  "Cipher Block Chaining",      "cbc",        0, 0
}, {
  A(OFB),                       R(MUST),      R(MUST),
  "Output Feedback",            "ofb",        0, 0
}, {
  A(CTR),                       R(MUST),      R(MUST),
  "Counter",                    "ctr",        0, 0
}, {
  A(STREAM),                    R(MUST),      R(MUST),
  "Stream",                     "stream",     0, 0
}, {
#undef A
...
#define A(a) T(COMPRESSION), PTPGP_COMPRESSION_TYPE_##a
  ...
  "Uncompressed",               "none",       0, 0
}, {
  A(ZIP),                       R(SHOULD),    R(SHOULD),
  "ZIP (RFC1951)",              "zip",        0, 0
}, {
  A(ZLIB),                      R(MAY),       R(MAY),
  "ZLIB (RFC1950)",             "zlib",       0, 0
}, {
  A(BZIP2),                     R(MAY),       R(MAY),
  "BZip2",                      "bzip2",      0, 0
}, FOOTER, {
...
#define A(a) T(HASH), PTPGP_HASH_TYPE_##a
  ...
  "MD5 (Deprecated)",           "md5",        128, 0
}, {
  A(SHA1),                      R(MUST),      R(MUST),
  "SHA-1",                      "sha1",       160, 0
}, {
  A(RIPEMD160),                 R(MAY),       R(MAY),
  "RIPE-MD/160",                "ripemd160",  160, 0
}, {
  ...
  "SHA-256",                    "sha256",     256, 0
}, {
  A(SHA384),                    R(MAY),       R(MAY),
  "SHA-384",                    "sha384",     384, 0
}, {
  A(SHA512),                    R(MAY),       R(MAY),
  "SHA-512",                    "sha512",     512, 0
}, FOOTER, {
#undef A
...
#define A(a) T(S2K), PTPGP_S2K_TYPE_##a
  ...
  "Iterated and Salted",        "iterated",   0, 0
}, FOOTER, {
#undef A
