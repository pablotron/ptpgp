...
#define OP_BASE64_BUFFER_SIZE     1024

typedef struct op_base64_t_ op_base64_t;

typedef enum {
  OP_OK,
  OP_ERR_BASE64_ALREADY_DONE,
  OP_ERR_LAST
} op_err_t;

typedef op_err_t (*op_base64_cb_t)(op_base64_t *, char *, size_t);
...
struct op_base64_t_ {
  ...
  char out_buf[OP_BASE64_BUFFER_SIZE];
  size_t out_buf_len;
...
#define DIE(p, e) do {                                            \
  return (p)->last_err = OP_ERR_BASE64_##e;                       \
} while (0)

#define FLUSH(p) do {                                             \
  if ((p)->out_buf_len > 0) {                                     \
    op_err_t err = (p)->cb((p), (p)->out_buf, (p)->out_buf_len);  \
    if (err) {                                                    \
      (p)->last_err = err;                                        \
      return err;                                                 \
    }                                                             \
...
op_err_t
op_base64_init(op_base64_t *p, char encode, op_base64_cb_t cb, void *user_data) {
  memset(p, 0, sizeof(op_base64_t));
...
op_err_t
op_base64_push(op_base64_t *p, char *src, size_t src_len) {
  size_t i;
  ...
  if (!src || !src_len) {
    ...
    if (p->src_buf_len > 0) {
      ...
      if (p->out_buf_len + 3 >= OP_BASE64_BUFFER_SIZE)
        FLUSH(p);
...
op_err_t
op_base64_done(op_base64_t *p) {
  return op_base64_push(p, 0, 0);
}
